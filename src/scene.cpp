        ...
        if (scene.contains("materials")) {
            for (const auto& materialData :
                ...
                if (materialData.is_string()) {
                    ...
                    if (ext == ".mtl" || ext == ".MTL") {
                        auto loaded = loadMaterialsFromMtl(matPath, resourceManager);

                        if (loaded.empty()) {
                            std::cerr << "[Error] No materials loaded from .mtl: " << matPath << std::endl;
                            return false;
                        }
                    ...
                    std::cerr << "[Error] Unsupported material file: " << matPath << std::endl;
                    return false;
                }
