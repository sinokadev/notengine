...
bool Scene::loadSeno(const std::string& path) {
    ...
    if (!file.is_open()) {
        std::cerr << "[Error] Failed to open Seno scene: " << path << std::endl;
        return false;
    }
    ...
    try {
        ...
        if (scene.contains("meshes")) {
            ...
            if (!meshArray.is_array()) {
                std::cerr << "[Error] 'meshes' must be an array" << std::endl;
                return false;
            }
            ...
            for (const auto& meshData : meshArray) {
                ...
                if (meshData.is_string()) {
                    ...
                    if (!mesh) {
                        std::cerr << "[Error] Failed to load mesh: " << meshPath << std::endl;
                        return false;
                    }
                }
                ...
                // Embedded mesh
                    ...
                    const std::string type = meshData.value("type", "embedded");

                    if (type != "embedded") {
                        std::cerr << "[Error] Unknown embedded mesh type: " << type << std::endl;
                        return false;
                    }
                    ...
                    for (const auto& vertexData : meshData["vertices"]) {
                        ...
                        vertex.Position = glm::vec3(position[0].get<float>(), position[1].get<float>(), position[2].get<float>());

                        if (vertexData.contains("normal")) {
                            const auto& normal = vertexData["normal"];

                            vertex.Normal = glm::vec3(normal[0].get<float>(), normal[1].get<float>(), normal[2].get<float>());
                        } else {
                            vertex.Normal = glm::vec3(0.0f, 1.0f, 0.0f);
                        }

                        if (vertexData.contains("texcoord")) {
                            const auto& uv = vertexData["texcoord"];

                            vertex.TexCoords = glm::vec2(uv[0].get<float>(), uv[1].get<float>());
                        } else {
                            vertex.TexCoords = glm::vec2(0.0f);
                        }

                        if (vertexData.contains("tangent")) {
                            const auto& tangent = vertexData["tangent"];

                            vertex.Tangent = glm::vec3(tangent[0].get<float>(), tangent[1].get<float>(), tangent[2].get<float>());
                        } else {
                    ...
                    for (const auto& index : meshData["indices"]) {
                        mesh->indices.push_back(index.get<unsigned int>());
                    }

                    if (mesh->vertices.empty() || mesh->indices.empty()) {
                        std::cerr << "[Error] Embedded mesh is empty" << std::endl;
                        return false;
                    }
                    ...
                    std::cerr << "[Error] Invalid mesh entry" << std::endl;
                    return false;
                }
        ...
        if (scene.contains("materials")) {
            for (const auto& materialData : scene["materials"]) {
                ...
                const std::string shaderName = materialData.value("shader", "");

                if (shaderName == "pbrShader") {
                    auto shader = resourceManager.getShader("pbrShader");

                    if (!shader) {
                        std::cerr << "[Error] Failed to find shader: " << shaderName << std::endl;
                        return false;
                    }
                    ...
                    if (materialData.contains("albedo")) {
                        ...
                        if (!c.is_array() || c.size() != 3) {
                            std::cerr << "[Error] Invalid albedo value" << std::endl;
                            return false;
                        }

                        albedo = glm::vec3(c[0].get<float>(), c[1].get<float>(), c[2].get<float>());
                    }

                    const float metallic = materialData.value("metallic", 0.0f);

                    const float roughness = materialData.value("roughness", 0.5f);

                    const float ao = materialData.value("ao", 1.0f);

                    materials.push_back(std::make_shared<PbrMaterial>(shader, albedo, metallic, roughness, ao));
                } else {
                    std::cerr << "[Error] Unknown shader: " << shaderName << std::endl;
                    return false;
                }
            }
        }
        ...
        if (scene.contains("models")) {
            for (const auto& modelData : scene["models"]) {
                ...
                const int meshIndex = modelData.value("mesh", -1);

                const int materialIndex = modelData.value("material", -1);

                if (meshIndex < 0 || meshIndex >= static_cast<int>(meshes.size())) {
                    std::cerr << "[Error] Model mesh index " << meshIndex << " is out of range" << std::endl;
                    return false;
                }

                if (materialIndex < 0 || materialIndex >= static_cast<int>(materials.size())) {
                    std::cerr << "[Error] Model material index " << materialIndex << " is out of range" << std::endl;
                    return false;
                }
        ...
        if (scene.contains("lights")) {
            for (const auto& lightData : scene["lights"]) {

                const std::string type = lightData.value("type", "");
                ...
                if (type == "PbrPointLight") {
                    ...
                    auto light = std::make_shared<DirLight>();
                    ...
                    if (lightData.contains("ambient")) {
                        const auto& c = lightData["ambient"];

                        light->ambient = glm::vec3(c[0].get<float>(), c[1].get<float>(), c[2].get<float>());
                    }

                    if (lightData.contains("diffuse")) {
                        const auto& c = lightData["diffuse"];

                        light->diffuse = glm::vec3(c[0].get<float>(), c[1].get<float>(), c[2].get<float>());
                    }

                    if (lightData.contains("specular")) {
                        const auto& c = lightData["specular"];

                        light->specular = glm::vec3(c[0].get<float>(), c[1].get<float>(), c[2].get<float>());
                    }

                    if (lightData.contains("rotation")) {
                        const auto& r = lightData["rotation"];

                        light->rotation = glm::quat(r[0].get<float>(), r[1].get<float>(), r[2].get<float>(), r[3].get<float>());
                    }
