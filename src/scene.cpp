    ...
    try {
        ...
        if (scene.contains("materials")) {
            for (const auto& materialData :
                ...
                // If a material entry is a string, treat it as an external file.
                if (materialData.is_string()) {
                    std::string matPath = resolveAssetPath(materialData.get<std::string>());

                    // Support .mtl files: parse basic Kd/Ns/map_Kd entries and create PBR materials.
                    auto extPos = matPath.find_last_of('.');
                    std::string ext = (extPos != std::string::npos) ? matPath.substr(extPos) : "";

                    if (ext == ".mtl" || ext == ".MTL") {
                        std::ifstream mtlFile(matPath);

                        if (!mtlFile.is_open()) {
                            std::cerr << "[Error] Failed to open MTL file: " << matPath << std::endl;
                            return false;
                        }
                        ...
                        while (std::getline(mtlFile, line)) {
                            ...
                            if (token == "newmtl") {
                                ...
                                // Tr is the inverse of d in some files
                                float tr = 0.0f;
                                if (iss >> tr) current.d = 1.0f - tr;
                            } else if (token == "map_Kd") {
                                std::string tex;
                                // map_Kd may have options; read the last token as filename
                                if (iss >> tex) {
                                    current.map_kd = tex;
                                }
                            }
                        ...
                        std::string baseDir;
                        auto lastSlash = matPath.find_last_of("/\\");
                        if (lastSlash != std::string::npos)
                            baseDir = matPath.substr(0, lastSlash);
                        ...
                        for (const auto& e : entries) {
                            ...
                            if (!e.map_kd.empty()) {
                                ...
                                // If the texture path is not absolute, resolve relative to the .mtl file.
                            ...
                            // Convert Ns (shininess) to a roughness-like factor. This is a heuristic.
                            ...
                            materials.push_back(
                                std::make_shared<PbrMaterial>(
                                    shader,
                                    e.kd,
                                    0.0f,        // metallic (not available in MTL)
                                    roughness,   // roughness derived from Ns
                                    1.0f,        // ao
                                    albedoTex    // albedo texture if present
                                )
                            );
                        }

                        continue; // next materialData
                    }
                }

                // Fallback: object-style material entries (existing behavior)
