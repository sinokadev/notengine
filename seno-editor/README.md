...
Knot의 `.seno` v8 장면을 편집하는 C++17 / Dear ImGui 데스크톱 에디터입니다.
3D 미리보기에는 Knot의 실제 렌더러를 사용합니다.
...
`{assetRoot}`는 이 저장소 루트입니다. 엔진과 미리보기 모두 상대 에셋 경로는 `.seno` 파일의 디렉터리를 기준으로 해석합니다.
다른 디렉터리로 Save As할 때 상대 경로는 자동 재작성되지 않으므로 JSON에서 조정하거나 `{assetRoot}` 또는 절대 경로를 사용하세요.
`models[].obj`, 외부 `meshes[]`, `sky.path`에 `model.obj`, `models/tree.obj`, `../shared/sky.hdr` 같은 상대 경로를 사용할 수 있습니다. 절대 경로와 `{assetRoot}` 참조는 기존 방식대로 처리합니다.
...
엔진과 에디터는 정수 `"version": 8`만 허용합니다. 버전 누락, v7 등 다른 버전, 문자열 `"8"`, 실수 `8.0`은 거부합니다.
기존 파일을 사용하려면 형식을 확인한 뒤 버전을 명시적으로 8로 변경하세요. 자동 변환하지 않습니다.

오브젝트의 `"pivot": [x, y, z]`는 로컬 좌표계의 회전·크기 변경 중심이며 생략하면 `[0, 0, 0]`입니다.
Inspector에서 세 값을 편집할 수 있고, pivot이 없는 오브젝트에는 **Add pivot** 버튼이 표시됩니다.
월드 변환은 `T(position) * T(pivot) * R * S * T(-pivot)`이므로 pivot의 월드 위치는 `position + pivot`입니다.
회전과 크기 변경이 없다면 pivot만 바꾸어도 메시 위치는 변하지 않습니다.
